(function () {
  ...
  function renderCarousel() {
    const slideContainer = document.querySelector('.services-wrapper-slides');
    slideContainer.innerHTML = slides[currentSlideIdx];
    if (window.innerWidth > 800) {
      const secondPetBoxIdx = currentSlideIdx + 1 >= slides.length ? 0 : currentSlideIdx + 1;
      slideContainer.innerHTML += slides[secondPetBoxIdx];
      if (window.innerWidth > 1000) {
        const thirdPetBoxIdx = secondPetBoxIdx + 1 >= slides.length ? 0 : secondPetBoxIdx + 1;
        slideContainer.innerHTML += slides[thirdPetBoxIdx];
      }
    }
  }
